...
- **Owner override 2026-08-17 (Supplement panel, final):** the panel has **no enclosing band or card at all** (the owner rejected a warm-gradient band after earlier rejecting a boxed packshot and an accent-bar card list): content sits directly on the shop-section background like the product grids. The header is slimmed to a single human-voiced panel H2 "You Know the Room." (set via the JS panel config; the description line is empty and hidden for this path) — the owner rejected the previous stack of panel heading + description + in-panel H3 as too many headings and too much copy. The body is one lead paragraph ("The one that never quite keeps up…") plus 13px confirm fine print and the two CTAs in the left column; the right column floats the Serena transparent cutout chromeless over a warm ground shadow; the three problem spaces ("The garage or workshop" / "The addition or converted space" / "That one room") sit as unboxed hairline-divided (`#dce4f3`) text columns across the full width, stacking to hairline rows on mobile. Recorded rules: **no boxes within boxes, no accent-bar card lists, no eyebrow-plus-card-list formula, no enclosing band, no stacked heading+description+H3 triple on this panel; imagery floats chromeless with a ground shadow, never in a tinted rectangle; "人味儿" for this owner means concrete second-person copy and grounded imagery, not added containers or colder layout mechanics.** The no-product-grid and compact-route rules for Supplement are unchanged.
